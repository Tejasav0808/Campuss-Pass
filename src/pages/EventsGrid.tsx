...
import { Search, Filter, Calendar, MapPin, ArrowLeft, Clock } from 'lucide-react';
      ...
      <main className="flex-grow pt-32 pb-20">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-12">
            <div>
              <h1 className="text-5xl font-bold text-slate-900 dark:text-white tracking-tight mb-4">Discover Events</h1>
              <p className="text-slate-500 dark:text-slate-400 text-lg max-w-2xl">Find your next obsession. Join workshops, festivals, and summits happening on campus.</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
              <div className="relative flex-grow lg:w-80">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-6 py-4 bg-white dark:bg-[#12092a] transition-colors duration-300 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm dark:shadow-none focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all"
                />
              </div>
              <div className="relative">
                <button 
                  onClick={() => setShowFilters(!showFilters)}
                  className={`flex items-center justify-center gap-2 px-8 py-4 border rounded-2xl font-bold transition-all shadow-sm dark:shadow-none ${
                    showFilters || priceFilter !== 'All' 
                      ? 'bg-indigo-600 text-white border-indigo-600 shadow-indigo-100' 
                      : 'bg-white dark:bg-[#12092a] transition-colors duration-300 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-[#1c2333] dark:bg-[#050014] transition-colors duration-300'
                  }`}
                >
                  <Filter size={20} />
                ...
                {showFilters && (
                  <>
                    ...
                    ></div>
                    <div className="absolute right-0 mt-4 w-72 bg-white dark:bg-[#12092a] transition-colors duration-300 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-800/60 p-6 z-30 animate-in fade-in zoom-in duration-200 origin-top-right">
                      <div className="space-y-6">
                              ...
                              <button
                                ...
                                className={`flex items-center justify-between px-4 py-3 rounded-xl text-sm font-bold transition-all ${
                                  priceFilter === f 
                                    ...
                                    : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-[#1c2333] dark:bg-[#050014] transition-colors duration-300'
                                }`}
                              >
                                {f}
                        ...
                        <div className="pt-6 border-t border-slate-50 dark:border-slate-800/40">
                ...
                <button
                  ...
                  className={`px-8 py-3 rounded-2xl text-sm font-bold whitespace-nowrap transition-all ${
                    activeCategory === cat 
                      ...
                      : 'bg-white dark:bg-[#12092a] transition-colors duration-300 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-[#1c2333] dark:bg-[#050014] transition-colors duration-300 border border-slate-100 dark:border-slate-800/60'
                  }`}
                >
                  {cat}
                </button>
                ...
                <button
                  ...
                  className={`px-5 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${
                    activeSubCategory === 'All' 
                      ...
                      : 'bg-slate-50 dark:bg-[#050014] transition-colors duration-300 text-slate-400 hover:bg-slate-100 dark:bg-slate-800 transition-colors duration-300'
                  }`}
                >
                  All {activeCategory}
                </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredEvents.map((event, index) => (
              ...
              >
                <Link 
                  to={`/events/${event.id}`} 
                  className="group block bg-white dark:bg-[#12092a] transition-colors duration-300 rounded-[2.5rem] overflow-hidden border border-slate-100 dark:border-slate-800/60 shadow-sm dark:shadow-none hover:shadow-2xl hover:-translate-y-2 transition-all duration-500"
                >
                  <div className="relative h-64 overflow-hidden">
                    ...
                    <div className="absolute top-6 right-6 flex flex-col gap-2">
                      <div className="bg-white dark:bg-[#12092a] transition-colors duration-300/90 backdrop-blur-md px-4 py-2 rounded-2xl text-[10px] font-black text-slate-900 dark:text-white shadow-xl uppercase tracking-tighter">
                        {event.category}
                      </div>
                  ...
                  <div className="p-8">
                    ...
                    <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-3 group-hover:text-indigo-600 transition-colors leading-tight">
                      {event.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-slate-50 dark:border-slate-800/40">
                      <div className="flex -space-x-3">
                        ...
                        <div className="w-8 h-8 rounded-full border-2 border-white bg-slate-100 dark:bg-slate-800 transition-colors duration-300 flex items-center justify-center text-[10px] font-bold text-slate-500 dark:text-slate-400">
                          +42
                        </div>
                      </div>
          ...
          {filteredEvents.length === 0 && (
            <div className="text-center py-20">
              <div className="w-20 h-20 bg-slate-100 dark:bg-slate-800 transition-colors duration-300 rounded-full flex items-center justify-center mx-auto mb-6">

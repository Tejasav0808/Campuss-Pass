...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const loadData = () => {
      ...
      JSON.parse(localStorage.getItem('campus_pass_global_registrations') || '[]');
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            <div>
              ...
              <h1 className="text-5xl font-black text-slate-900 dark:text-white tracking-tight mb-2">Command Hub</h1>
              <p className="text-slate-500 dark:text-slate-400 text-lg font-medium">Global oversight and platform orchestration.</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
            {[
              ...
              <motion.div 
                ...
                key={stat.label} 
                className="bg-white dark:bg-[#12092a] transition-colors duration-300 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800/60 shadow-sm dark:shadow-none hover:shadow-md transition-shadow relative overflow-hidden group"
              >
                ...
                <div className="flex items-end justify-between mb-1">
                  ...
                  <div className={`text-xs font-bold px-2 py-1 rounded-lg ${stat.change.includes('new') || stat.change.includes('+') ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-50 dark:bg-[#050014] transition-colors duration-300 text-slate-400'}`}>
                    {stat.change}
                  </div>
                </div>
          ...
          {/* Main Content Area */}
          <div className="bg-white dark:bg-[#12092a] transition-colors duration-300 rounded-[3rem] border border-slate-100 dark:border-slate-800/60 shadow-sm dark:shadow-none overflow-hidden flex flex-col min-h-[600px]">
            {/* Tab Navigation */}
            <div className="flex flex-col md:flex-row md:items-center justify-between px-10 pt-10 border-b border-slate-50 dark:border-slate-800/40 gap-6">
              <div className="flex items-center gap-8">
                {[
                  ...
                  <button 
                    ...
                    className={`pb-8 text-xs font-black tracking-[0.2em] uppercase transition-all relative flex items-center gap-2 ${activeTab === tab.id ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600 dark:text-slate-300'}`}
                  >
                    <tab.icon size={14} />
                    {tab.label}
                    {activeTab === tab.id && (
              ...
              <div className="pb-8 flex items-center gap-4">
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="pl-12 pr-6 py-3 bg-slate-50 dark:bg-[#050014] transition-colors duration-300 border border-slate-100 dark:border-slate-800/60 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 w-64"
                  />
                </div>
                <button className="p-3 bg-slate-50 dark:bg-[#050014] transition-colors duration-300 text-slate-500 dark:text-slate-400 rounded-2xl hover:bg-slate-100 dark:bg-slate-800 transition-colors duration-300 transition-colors">
                  <Filter size={18} />
                </button>
              </div>
            </div>
                  ...
                  >
                    ...
                    <div className="space-y-8">
                      <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
                        <div className="absolute bottom-0 right-0 w-32 h-32 bg-white dark:bg-[#12092a] transition-colors duration-300/5 rounded-full -mr-16 -mb-16 group-hover:scale-125 transition-transform duration-700" />
                        <h3 className="text-xl font-bold mb-6">Pending Approvals</h3>
                        <div className="space-y-4">
                          {[1, 2].map((i) => (
                            <div key={i} className="bg-white dark:bg-[#12092a] transition-colors duration-300/5 p-4 rounded-2xl border border-white/10">
                              ...
                              <div className="flex gap-2">
                                ...
                                <button className="px-3 py-2 bg-white dark:bg-[#12092a] transition-colors duration-300/5 text-slate-400 rounded-xl hover:text-white transition-colors"><XCircle size={14} /></button>
                              </div>
                            </div>
                          ))}
                        ...
                        <tbody>
                          {events.length === 0 ? (
                            ...
                            events.filter(e => e.title.toLowerCase().includes(searchQuery.toLowerCase())).map((ev) => (
                              <tr key={ev.id} className="group hover:bg-slate-50 dark:hover:bg-[#1c2333] dark:bg-[#050014] transition-colors duration-300 transition-all bg-white dark:bg-[#12092a] transition-colors duration-300 shadow-sm dark:shadow-none ring-1 ring-slate-100 rounded-3xl">
                                <td className="px-6 py-5 rounded-l-[1.5rem]">
                                  <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 bg-slate-100 dark:bg-slate-800 transition-colors duration-300 rounded-xl overflow-hidden flex-shrink-0 group-hover:scale-105 transition-transform">
                                ...
                                <td className="px-6 py-5">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm font-bold text-slate-600 dark:text-slate-300">{ev.category}</span>
                                  </div>
                                </td>
                                <td className="px-6 py-5 text-center">
                                  <div className="font-black text-slate-900 dark:text-white">{ev.registered || 0}</div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {organizers.length === 0 ? (
                        ...
                        organizers.filter(o => o.name.toLowerCase().includes(searchQuery.toLowerCase())).map((org) => (
                          <div key={org.id} className="bg-white dark:bg-[#12092a] transition-colors duration-300 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800/60 shadow-sm dark:shadow-none hover:shadow-md transition-shadow group">
                            <div className="flex items-start justify-between mb-6">
                              ...
                              <button className="p-2 text-slate-300 hover:text-slate-600 dark:text-slate-300 transition-colors"><MoreVertical size={20} /></button>
                            </div>
                            ...
                            <div className="flex items-center justify-between pt-6 border-t border-slate-50 dark:border-slate-800/40">
                              <div className="flex flex-col">
                                ...
                                <span className="font-bold text-slate-700 dark:text-slate-200">Organizer</span>
                              </div>
                        ...
                        <tbody>
                          {allUsers.length === 0 ? (
                            ...
                            allUsers.filter(u => u.name.toLowerCase().includes(searchQuery.toLowerCase()) || u.email.toLowerCase().includes(searchQuery.toLowerCase())).map((u) => (
                              <tr key={u.id} className="group hover:bg-slate-50 dark:hover:bg-[#1c2333] dark:bg-[#050014] transition-colors duration-300 transition-all bg-white dark:bg-[#12092a] transition-colors duration-300 shadow-sm dark:shadow-none ring-1 ring-slate-100 rounded-3xl">
                                <td className="px-6 py-5 rounded-l-[1.5rem]">
                                  <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 bg-slate-100 dark:bg-slate-800 transition-colors duration-300 rounded-xl flex items-center justify-center font-bold text-slate-400">
                                ...
                                <td className="px-6 py-5">
                                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter ${
                                    ...
                                    'bg-slate-50 dark:bg-[#050014] transition-colors duration-300 text-slate-500 dark:text-slate-400'
                                  }`}>
                                    {u.role}
                                  </span>
                                </td>
                      ...
                      <table className="w-full text-left border-separate border-spacing-y-4 -mt-4">
                        ...
                        <tbody>
                          {logs.length === 0 ? (
                            ...
                            logs.map((log) => (
                              <tr key={log.id} className="group hover:bg-slate-50 dark:hover:bg-[#1c2333] dark:bg-[#050014] transition-colors duration-300 transition-all bg-white dark:bg-[#12092a] transition-colors duration-300 shadow-sm dark:shadow-none ring-1 ring-slate-100 rounded-3xl">
                                <td className="px-6 py-5 rounded-l-[1.5rem]">
                                  <div className="flex items-center gap-4">
                                ...
                                <td className="px-6 py-5">
                                  <div className="text-sm font-bold text-slate-700 dark:text-slate-200">{log.email}</div>
                                  <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-0.5">Roll: {log.rollNumber}</div>
                                </td>
                                <td className="px-6 py-5">
                                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter ${
                                    log.role === 'organizer' ? 'bg-violet-50 text-violet-600' : 'bg-slate-100 dark:bg-slate-800 transition-colors duration-300 text-slate-500 dark:text-slate-400'
                                  }`}>
                                    {log.role}
                                  </span>
                                </td>
            ...
            >
              <div className="p-10">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <button 
                    onClick={() => setIsInviteModalOpen(false)}
                    className="p-2 text-slate-400 hover:text-slate-900 dark:text-white transition-colors"
                  >
                    <XCircle size={24} />
                  </button>
                </div>
                
                <h3 className="text-3xl font-black text-slate-900 dark:text-white mb-2">Invite Organizer</h3>
                <p className="text-slate-500 dark:text-slate-400 mb-8 font-medium">Send an invitation link to a club or society representative.</p>
                ...
                <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setIsInviteModalOpen(false); alert('Invitation sent!'); }}>
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full px-6 py-4 bg-slate-50 dark:bg-[#050014] transition-colors duration-300 border border-slate-100 dark:border-slate-800/60 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all font-medium"
                      required
                    />
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-14 pr-6 py-4 bg-slate-50 dark:bg-[#050014] transition-colors duration-300 border border-slate-100 dark:border-slate-800/60 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all font-medium"
                        required
                      />
                    </div>
                  </div>

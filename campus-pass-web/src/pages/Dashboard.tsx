      ...
      <main className="flex-grow pt-32 pb-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
            <div>
              <h1 className="text-4xl font-black text-slate-900 tracking-tighter mb-2">Organizer Dashboard</h1>
              <p className="text-slate-500 text-lg font-medium">Welcome back, <span className="text-indigo-600">{user?.name}</span></p>
            </div>
            <Link to="/create-event" className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-bold shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all flex items-center gap-3">
              <Plus size={20} />
              New Event
            </Link>
          </div>
                  ...
                  {events.length === 0 ? (
                    ...
                    events.map((ev) => (
                      <div 
                        ...
                        className="flex items-center gap-6 p-4 rounded-2xl border border-slate-50 hover:border-indigo-100 transition-colors cursor-pointer group"
                      >
                        <div className="w-16 h-16 bg-slate-50 rounded-xl flex-shrink-0 flex items-center justify-center text-slate-300 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors">
                          <BarChart3 size={32} />
                        </div>
                        <div className="flex-grow">
                          <div className="font-bold text-slate-900 text-lg group-hover:text-indigo-600 transition-colors">{ev.title}</div>
                          <div className="text-sm text-slate-400 font-medium">{ev.registered} Registered • {ev.date}</div>
                        </div>
                        <div className="flex items-center gap-2">
                          <button 
                            className="w-10 h-10 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-indigo-600 hover:text-white transition-all"
              ...
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                ...
                {events.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="font-bold text-slate-900 mb-1 group-hover:text-indigo-600 transition-colors">{events[0].title}</div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-grow pt-32 pb-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
            <div>
              <h1 className="text-4xl font-black text-slate-900 tracking-tighter mb-2">Hello, {user?.name} 👋</h1>
              <p className="text-slate-500 text-lg font-medium">Find your next favorite campus event.</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <Link to="/registrations" className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
              <div className="w-14 h-14 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                <Calendar size={28} />
              </div>
              <h3 className="text-2xl font-black text-slate-900 mb-2">Registered Events</h3>
              <p className="text-slate-500 font-medium leading-relaxed">View your upcoming events and past registrations history.</p>
            </Link>
            <Link to="/events" className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
              <div className="w-14 h-14 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                <Compass size={28} />
              </div>
              <h3 className="text-2xl font-black text-slate-900 mb-2">Explore Events</h3>
              <p className="text-slate-500 font-medium leading-relaxed">Discover upcoming workshops, seminars, and social activities on campus.</p>
            </Link>
          </div>
        </div>
      </main>

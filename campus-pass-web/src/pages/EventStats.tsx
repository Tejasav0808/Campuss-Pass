...
import { 
  ...
  Download, Search, Filter, Mail,
  Phone, User as UserIcon, GraduationCap,
  Clock, Hash
} from 'lucide-react';
...
export default function EventStats() {
  ...
  const handleExport = () => {
    ...
    const csvContent = "data:text/csv;charset=utf-8," 
      ...
      + registrations.map(r => `${r.fullName},${r.rollNo},${r.email},${r.branch},${r.semester},${r.status}`).join("\n");
      ...
      <main className="flex-grow pt-32 pb-20">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="mb-12">
            <button 
              onClick={() => navigate(-1)} 
              className="inline-flex items-center gap-2 text-slate-500 hover:text-indigo-600 transition-colors font-bold group mb-6"
            >
              <ArrowLeft size={18} />
              Back to Dashboard
            </button>
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div>
                ...
                <h1 className="text-5xl font-black text-slate-900 tracking-tighter mb-4">{event.title}</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
              ...
              <div className="text-4xl font-black text-slate-900 mb-2">{registrations.length}</div>
            ...
            <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="relative w-full md:w-80">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-6 py-3 bg-slate-50 border border-slate-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all font-medium text-sm"
                />
              </div>
            </div>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {filteredRegistrations.map((reg) => (
                      <tr key={reg.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-2 text-sm font-bold text-slate-600 bg-slate-100/50 px-3 py-1.5 rounded-lg w-fit">
                        ...
                        <td className="px-8 py-6">
                          <div>
                            <div className="text-sm font-bold text-slate-700">{reg.student_branch}</div>
